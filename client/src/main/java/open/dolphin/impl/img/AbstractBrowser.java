...
//s.oh$
    ...
    protected ImageEntry getEntryAt(int row, int col) {
        if (row != -1 && col != -1) {
            return (ImageEntry) tableModel.getValueAt(row, col);
        }
        return null;
    }
        ...
        SwingWorker worker = new SwingWorker<ArrayList<ImageEntry>, Void>() {
            ...
            protected ArrayList<ImageEntry> doInBackground() throws Exception {
                ...
                if (sortIsLastModified()) {
                    // 最終更新日でソート
                    if (sortIsDescending()) {
                        paths.sort((final Object o1, final Object o2) -> {
                            try {
...
//s.oh$
                    ...
                    for (String acceptImageType : ACCEPT_IMAGE_TYPES) {
                        if (acceptImageType.equals(suffix)) {
//s.oh^ 2014/05/07 PDF・画像タブの改善
