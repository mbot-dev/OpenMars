...
public class SendClaimImpl implements ClaimMessageListener {
    ...
    private void setup() {
        ...
        if (orcaSocket == null) {
            orcaSocket = new OrcaSocket(getHost(), getPort(), DEFAULT_SLEEP_TIME, tryCount);
        }
    }
    ...
    public void logDump() {
        ...
        synchronized(queue) {
            ...
            for (Object o : queue) {
                ClaimMessageEvent evt = (ClaimMessageEvent) o;
                Logger.getLogger(this.getClass().getName()).warning(evt.getClaimInsutance());
            }
...
//        int TT_NAK_SIGNAL = 1;// Alert constants
//        int TT_QUEUE_SIZE = 0;
        switch(code) {
