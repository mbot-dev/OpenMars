...
//s.oh^ 2013/06/13 カラムの並び順
        ...
        if(!Project.getBoolean("labtest.column.newest.left", false)) {
            modules.sort(new SampleDateComparator());
        }
//s.oh$
...
//s.oh^ 2013/03/19 機能改善(カラムを同一横幅)
    ...
    private void initialize() {
        ...
        // Divider
        int dividerLoc = DEFAULT_DIVIDER_LOC;
...
//s.oh$
        ...
        splitPane.setDividerSize(DEFAULT_DIVIDER_WIDTH);
    ...
    private void searchLaboTest(final int firstResult) {
        ...
        DBTask task = new DBTask<List<NLaboModule>, Void>(getContext()) {
            ...
            protected List<NLaboModule> doInBackground() throws Exception {
                return ldl.getLaboTest(pid, firstResult, getMaxResult());
            }
    ...
    private void deleteLabTest(final long moduleId) {
        ...
        DBTask task = new DBTask<Integer, Void>(getContext()) {
            ...
            protected Integer doInBackground() throws Exception {
                return ldl.deleteLabTest(moduleId);
            }
...
//s.oh^ ラボテストのPDF出力
        ...
        JButton pdfBtn = new JButton(buttonText);
        ...
        // ラボデータPDFの印刷
        JButton printPdfBtn = new JButton(buttonText);
...
//s.oh^ ラボテストのPDF出力    
    ...
    private String createPDF(boolean showDlg) {
        if(showDlg) {
            ...
            LaboTestOutputPDF pdf = new LaboTestOutputPDF(getContext().getPatient().getPatientId(), getContext().getPatient().getFullName(), null, ClientContext.getTempDirectory(), table, chart);
            return pdf.create();
        }
        return null;
    }
