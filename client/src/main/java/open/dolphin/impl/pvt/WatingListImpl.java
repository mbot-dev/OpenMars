...
//s.oh$
    ...
    private void setup() {
        ...
        // 受付番号カラム
        int numberColumn = columnHelper.getColumnPosition("getNumber");
...
//minagawa$            
        ...
        BitAndIconPair[] stateComboArray = new BitAndIconPair[userBitArray.length];
...
//s.oh$
    ...
    private void initComponents() {
        ...
        pvtTableModel = new ListTableModel<>(columnNames, 0, methods, cls) {
            ...
            public boolean isCellEditable(int row, int col) {
                ...
                if (pvt.getStateBit(PatientVisitModel.BIT_CANCEL)) {
                    ...
                    // Chartビットがたっている場合は不可
                    for (Integer integer : chartBitArray) {
                        if (pvt.getStateBit(integer)) {
...
//s.oh$
                        ...
                        String msg = pvt.getPatientName() + "様の受付をキャンセルしますか?";
...
//s.oh$
    ...
    private void connect() {
        ...
        view.getKutuBtn().addActionListener(e -> {
...
//minagawa$                
    ...
    private void updatePvtInfo() {
        ...
        sb.append(" | ");
        sb.append("来院数");
        sb.append(totalPvtCount);
        sb.append(" 待ち");
        sb.append(waitingPvtCount);
        sb.append(" 待時間 ");
...
//pns^
    ...
    private void countPvt() {
        ...
        for (PatientVisitModel pvt : dataList) {
...
//s.oh^ 2014/08/19 受付バーコード対応
    public void receiptBarcode() {
        ...
        open.addActionListener(ae -> openReceipt(infoField.getText()));
    ...
    public void openReceipt(String receipt) {
        ...
        if(info.length == 1) {
            ...
            for(int i = 0; i < pvtTableModel.getObjectCount(); i++) {
                ...
                if(pvt.getPatientId().equals(info[0]) && pvt.getFirstInsurance().contains(info[1]) && pvt.getDeptName().equals(info[2])) {
                    getContext().openKarte(pvt);
                    return;
                }
            }
