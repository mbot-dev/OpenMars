...
public final class DocumentDelegater extends BusinessDelegater {
    ...
    public KarteBean getKarte(long patientPK, Date fromDate) throws Exception {
        ...
        String path = "/karte/" +
                patientPK +
                CAMMA +
                new SimpleDateFormat(DATE_TIME_FORMAT_REST).format(fromDate);
        ...
        if (memoList!=null && memoList.size()>0) {
            memoList.stream().forEach((pm) -> pm.setKarteBean(karte));
        }
        return karte;
    }
    ...
    private List getKarteList(DocumentSearchSpec spec) throws Exception {
        ...
        // PATH
        String path = "/karte/docinfo/" +
                spec.getKarteId() +
                CAMMA +
                new SimpleDateFormat(DATE_TIME_FORMAT_REST).format(spec.getFromDate()) +
                CAMMA +
                spec.isIncludeModifid();
...
//minagawa$                
    ...
    public int updateTitle(DocInfoModel docInfo) throws Exception {
        ...
        // body
        byte[] data = docInfo.getTitle().getBytes(StandardCharsets.UTF_8);
        
        // PUT with text
        String cntStr = putEasyText(path, data);
    ...
    public List<RegisteredDiagnosisModel> getDiagnosisList(long karteId, Date fromDate, boolean activeOnly) throws Exception {
        ...
        String path = "/karte/diagnosis/" +
                karteId + CAMMA +
                new SimpleDateFormat(DATE_TIME_FORMAT_REST).format(fromDate) + CAMMA +
                activeOnly;
...
//s.oh^ 2014/04/03 サマリー対応
    public PatientFreeDocumentModel getPatientFreeDocument(String id) throws Exception {
        ...
        return getEasyJson(path, PatientFreeDocumentModel.class);
    }
