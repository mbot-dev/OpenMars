...
//minagawa$        
        ...
        prePanel.getChkPatientInfo().addActionListener(e -> chkPatientInfo = !chkPatientInfo);

        /* 麻薬施用者免許証番号転記チェックボックスアクションリスナー設定 */
        prePanel.getChkUseDrugInfo().addActionListener(e -> {
            if (chkUseDrugInfo) {
        ...
        prePanel.getChkHomeMedical().addActionListener(e -> {
            if (chkHomeMedical) {
        ...
        // 一般名出力
        prePanel.getGeneralChk().addActionListener(ae -> Project.setBoolean("prescription.output.general",prePanel.getGeneralChk().isSelected()));
...
// ********** @009 2010/07/01 ↓↓ **********
    ...
    private int chkDate() {
        ...
        try {       
            ...
            /** 現在日 */
            // @009
            Date now = Utils.chkDate(Utils.getDateFormat().format(new Date())); // 現在日
            // 使用期間入力チェック
            period = null;
    ...
    private void createPDF() {
        ...
        final SwingWorker worker = new SwingWorker<String,Void>() {
            ...
            protected String doInBackground() throws Exception {
                ...
                List<BundleMed> allList = new ArrayList<>(naiyoList.size() + otherList.size());
