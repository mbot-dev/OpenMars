...
public class KartePDFMaker2 extends AbstractLetterPDFMaker {
    ...
    public void initKartePDF() {
        if(bCreating) addDataEnd();
        if(bInitialized) create();
        try {
            ...
            if(!getDocumentDir().endsWith(File.separator)) {
...
//s.oh^ 2013/02/07 印刷対応
            ...
            String sbFooter = "【" +
                    patName +
                    " " +
                    ClientContext.getMyBundle(KartePDFMaker2.class).getString("person.title") +
                    "】" +
                    " Page ";
            HeaderFooter footer = new HeaderFooter(new Phrase(sbFooter, bodyFont), true);
//s.oh$
    ...
    public void addData(String data, int fontSize, int style, Color color) {
        if(!bInitialized || !bCreating) return;
