...
public class KartePDFMaker extends AbstractPDFMaker {
    ...
    public boolean makePDF(String filePath) {
        ...
        if (ascending) {
            ...
            docList.sort(Collections.reverseOrder());
        }
...
//minagawa$            
            ...
            // フッターに名前とIDを入れる
            String sb = getPatientId() + " " +
                    getPatientName() + bundle.getString("text.space_personTitle") +
                    FRMT_DATE_WITH_TIME.format(new Date()) +
                    "  Page ";
            HeaderFooter footer = new HeaderFooter(new Phrase(sb, font), true);
    ...
    private KarteTable createTable(DocumentModel model, int col) {
        ...
        String bookmark = FRMT_DATE_WITH_TIME.format(model.getDocInfoModel().getFirstConfirmDate()) +
                "\n" +
                model.getDocInfoModel().getTitle();
    ...
    private String parseBundleNum(String str) {
        ...
        String sb = bundle.getString("text.numTimes_colon") +
                str.substring(0, pos) +
                bundle.getString("text.space_performDate_colon") +
                str.substring(pos + 1, len) +
                bundle.getString("text.day");
        return sb;
    }
        ...
        private PdfPTable createStampTable(ModuleModel stamp) {
            ...
            try {
                ...
                if (IInfoModel.ENTITY_LABO_TEST.equals(entity)) {
                    ...
                    // スタンプ名
                    String sb = model.getOrderName() +
                            "(" +
                            stamp.getModuleInfoBean().getStampName() +
                            ")";
                    table.addCell(createStampCell(sb, 2, true));
                    // ClassCode
