...
public class LetterImpl extends AbstractChartDocument implements Letter {
    ...
    public void modelToView(LetterModule m) {
        ...
        //s.oh^ 不具合修正
        LetterHelper.setModelValue(view.getDisease(), Objects.requireNonNullElse(value, ""));
//s.oh$

        // 紹介目的
        value = model.getItemValue(ITEM_PURPOSE);
        //s.oh^ 不具合修正
        LetterHelper.setModelValue(view.getPurpose(), Objects.requireNonNullElse(value, ""));
//s.oh$

        // 既往歴、家族歴
        String text = model.getTextValue(TEXT_PAST_FAMILY);
        //s.oh^ 不具合修正
        LetterHelper.setModelValue(view.getPastFamily(), Objects.requireNonNullElse(text, ""));
//s.oh$

        // 症状経過
        text = model.getTextValue(TEXT_CLINICAL_COURSE);
        //s.oh^ 不具合修正
        LetterHelper.setModelValue(view.getClinicalCourse(), Objects.requireNonNullElse(text, ""));
//s.oh$

        // 現在の処方
        text = model.getTextValue(TEXT_MEDICATION);
        //s.oh^ 不具合修正
        LetterHelper.setModelValue(view.getMedication(), Objects.requireNonNullElse(text, ""));
//s.oh$

        // 備考
        value = model.getItemValue(ITEM_REMARKS);
        //s.oh^ 不具合修正
        LetterHelper.setModelValue(view.getRemarks(), Objects.requireNonNullElse(value, ""));
//s.oh$
    }
...
//s.oh$
        ...
        // Title
        model.setTitle(ClientContext.getMyBundle(LetterImpl.class).getString("refferalLetter") + model.getConsultantHospital());
    }
...
//s.oh$
    ...
    public void print() {
        ...
        if (option == 0) {
            ...
            Panel2 panel = this.view;
            panel.printPanel(pageFormat, 1, name, 0, true);
        }
    }
