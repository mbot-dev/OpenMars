...
//s.oh$
        ...
        if (ClientContext.isMac()) {
            ...
            // About
            desktop.setAboutHandler((java.awt.desktop.AboutEvent ae) -> showAbout());

            // Preference
            desktop.setPreferencesHandler((java.awt.desktop.PreferencesEvent pe) -> doPreference());

            // Quit
            desktop.setQuitHandler((java.awt.desktop.QuitEvent qe, java.awt.desktop.QuitResponse qr) -> processExit());
...
//masuda$         
    ...
    private void startSendClaim() {
        ...
        if (iter.hasNext()) {
            // CLAIM リスナ
            ClaimMessageListener sendClaim = iter.next();
    ...
    private void startSendMml() {
        ...
        if (iter.hasNext()) {
            // MML リスナ
            MmlMessageListener sendMml = iter.next();
...
//minagawa^ First Commit Win Control
                ...
                if (test!=null && test.contains("First Commit Win")) {
                    SwingUtilities.invokeLater(() -> doFirstCommitWinAlert(treeTosave));
                    
                } else {
                    SwingUtilities.invokeLater(() -> doStoppingAlert());
                }
//minagawa$                
...
//s.oh^ 2014/07/22 一括カルテPDF出力
    ...
    private void outputAllKartePdfForPatient(PatientModel pm) {
        ...
        String sb = ClientContext.getTempDirectory() +
                File.separator +
                pm.getPatientId();
        File dir = new File(sb);
        if(!dir.exists()) {
            dir.mkdirs();
        }
        try {
    ...
    private void outputPdf(PatientModel pm, DocumentModel model, String dir) {
        if(model.getModules() != null) {
            ...
            ArrayList<ModuleModel> soaModules = new ArrayList<>();
            ArrayList<ModuleModel> pModules = new ArrayList<>();
            String soaSpec = null;
            String pSpec = null;
            ...
            dumper.setModuleList(soaModules);
            ...
            pdumper.setModuleList(pModules);
