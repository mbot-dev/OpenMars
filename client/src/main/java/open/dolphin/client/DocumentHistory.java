...
public class DocumentHistory {
    ...
    private void updateHistory(List<DocInfoModel> mewHistory) {
        ...
        if (mewHistory != null && mewHistory.size() > 0) {
            if (ascending) {
                ...
                mewHistory.sort(Collections.reverseOrder());
            }
        }
    ...
    private void initComponent() {
        ...
        // 文書履歴テーブルを生成する
        tableModel = new ListTableModel<>(columnNames, 0, methodNames, columnClasses) {
            ...
            if (!e.getValueIsAdjusting()) {
                ...
                if (selectedRows.length > 0) {
                    ...
                    for (int selectedRow : selectedRows) {
                        DocInfoModel obj = tableModel.getObject(selectedRow);
                        if (obj != null) {
                            list.add(obj);
                        }
                    }
...
//minagawa$        
    ...
    class DocInfoTask extends DBTask<List<DocInfoModel>, Void> {
        ...
        protected List<DocInfoModel> doInBackground() throws Exception {
            return (List<DocInfoModel>) ddl.getDocumentList(spec);
        }
