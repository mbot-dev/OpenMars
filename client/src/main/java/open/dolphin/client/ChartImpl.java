    ...
    public void start() {
        ...
        final SimpleWorker worker = new SimpleWorker<KarteBean, Void>() {
            ...
            protected KarteBean doInBackground() throws Exception {
                ...
                return ddl.getKarte(getPatientVisit().getPatientModel().getId(), today.getTime());
            }
            ...
            protected void succeeded(KarteBean karteBean) {
                ...
                //-------------------------------------------------------------
                initComponents();
                SwingUtilities.invokeLater(() -> getDocumentHistory().showHistory());
            }
...
//s.oh^ 他プロセス連携(アイコン) 2014/05/09
        if(Project.getBoolean(GUIConst.ACTION_OTHERPROCESS_ICON, false)) {
            ...
            for(int i = 1; i < num+1; i++) {
                final String KEY_DEF = "otherprocessicon" + i + ".link";
                ...
                linkBtn.addActionListener((ActionEvent e) -> DefaultBrowserEx.otherProcess(KEY_DEF, chart, Project.getString(KEY_DEF + ".path"), Project.getString(KEY_DEF + ".param"), null));
...
//s.oh$
    ...
    private void copyModel(DocumentModel oldModel, DocumentModel newModel, boolean applyRp) {
        ...
        if (applyRp) {
            ...
            List<ModuleModel> modules = oldModel.getModules();
            if (modules!=null) {
                modules.stream().forEach((bean) -> newModel.addModule(ModelUtils.cloneModule(bean)));
            }
            List<SchemaModel> schema = oldModel.getSchema();
            if (schema!=null) {
                schema.stream().forEach((scm) -> newModel.addSchema(ModelUtils.cloneSchema(scm)));
            }
...
//minagawa$    
    ...
    public void newDocument() {
        ...
        if (nproviders != null) {
            for (String cmd : nproviders.keySet()) {
    ...
    public void stop() {
        ...
        SwingWorker worker = new SwingWorker<Integer, Void>() {
            ...
            protected Integer doInBackground() throws Exception {
                ...
                return scl.publishKarteClosedInWorkerThread(ChartImpl.this.getPatientVisit());
            }
