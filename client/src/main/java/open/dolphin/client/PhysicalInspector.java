...
//s.oh$        
                ...
                private void mabeShowPopup(MouseEvent e) {
                    if (e.isPopupTrigger()) {
                        ...
                        if (tableModel.getObject(row) != null) {
                            ...
                            pop.add(item3);
                            item3.addActionListener((ActionEvent e1) -> delete(row));
                        }
    ...
    public void update() {
        ...
        if (listH != null && listW != null) {
            ...
            for (PhysicalModel model : listH) {

                PhysicalModel h = (PhysicalModel) model;
                ...
                for (PhysicalModel physicalModel : listW) {
                    PhysicalModel w = (PhysicalModel) physicalModel;
            ...
            // 体重のリストが残っていればループする
            if (listW.size() > 0) {
                listW.stream().forEach(list::add);
            }
            
        } else if (listH != null) {
            // 身長だけの場合
            listH.stream().forEach(list::add);
    ...
    public void add(final PhysicalModel model) {
        ...
        DBTask task = new DBTask<List<Long>, Void>(context) {
            ...
            protected List<Long> doInBackground() throws Exception {
                ...
                return pdl.addObservations(addList);
            }
