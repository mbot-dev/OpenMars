...
//s.oh$
    ...
    public void stop() {
        if (karteList != null) {
            karteList.stream().forEach(KarteViewer::stop);
            karteList.clear();
        }
    }
...
//s.oh$
    ...
    public void showDocuments(DocInfoModel[] selectedHistories, final JScrollPane scroller) {
        ...
        if (removed != null && removed.size() > 0) {
            removed.stream().forEach((karte) -> karteList.remove(karte));
        }
...
//s.oh$
        ...
        if (added.isEmpty()) {
            ...
            karteList.stream().forEach((view) -> scrollerPanel.add(view.getUI()));
...
//s.oh$
        ...
        added.stream().forEach((bean) -> docId.add(bean.getDocPk()));
...
//s.oh$
            ...
            if (ascending) {
                ...
                karteList.sort(Collections.reverseOrder());
            }
...
//            }
            ...
            getContext().showDocument(0);
            if (removed != null) {
                removed.stream().forEach(KarteViewer::stop);
                removed.clear();
            }
        };
